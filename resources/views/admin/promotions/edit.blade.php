        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" id="form" action="{{ route('promotions.update', $promotion) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="title" :value="__('Заголовок')" />
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" value="{{ old('name', $promotion->title) }}" />
                            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="photo" :value="__('Фото')" />
                            <img src="{{ asset($promotion->photo) }}" style="height: 100px;">
                        ...
                        <div>
                            ...
                            <div id="editor" name="editor" class="mt-1 block w-full">{!! old('description', $promotion->description) !!}</div>
